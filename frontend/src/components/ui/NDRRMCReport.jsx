...
const NDRRMCReport = () => {
  const { ndrrmcReport, isReportVisible, setReportVisible } = useArkStore(
    useShallow((state) => ({
  ...
  const targetText = useMemo(() => {
    if (!ndrrmcReport) return '';
    if (typeof ndrrmcReport === 'string') return ndrrmcReport;
    return language === 'EN' ? (ndrrmcReport.en || '') : (ndrrmcReport.fil || '');
  }, [ndrrmcReport, language]);
  ...
  const renderLines = () => {
    return displayedText.split('\n').map((line, idx) => {
      ...
      return <div key={idx} className={`${colorClass} min-h-[1rem] leading-tight`}>{line}</div>;
    });
  };
    ...
    >
      <div className="flex items-center justify-between p-3 border-b border-ark-border">
        ...
        <div className="flex space-x-3">
          <button onClick={handleCopy} className="text-ark-silver hover:text-cyan-400 transition-colors" title="Copy">
            {copied ? '✓' : '📋'}
          </button>
          <button onClick={() => setReportVisible(false)} className="text-ark-silver hover:text-rose-500 font-bold transition-colors" title="Close">
            ✕
          </button>
        </div>
      </div>
          ...
          {['EN', 'FIL'].map((lang) => (
            <button
              ...
              className={`px-2 py-0.5 text-[10px] font-mono border rounded-sm transition-colors ${
                language === lang ? 'bg-cyan-400/20 text-cyan-400 border-cyan-400' : 'text-ark-silver border-ark-border hover:border-ark-silver'
              }`}
            >
              {lang}
            </button>
